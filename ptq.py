...
def ptdq(config, model):
    '''
    PTDQ必须保证在量化转换前将模型搬到CPU,且不支持conv的量化
    '''
    model.to('cpu')
...
def ptsq(config, model, test_dataloader):
    """
    PTSQ必须保证在量化转换前将模型搬到CPU, 且在convert之前必须跑一边测试，来给observer收集数据
    PTSQ不支持加减乘除操作，因此必须在原模型中改掉
    """
    model.to('cpu')
    quant_model = ResNet18_quant(num_classes=config['num_classes']).to('cpu')
    ...
    #必须跑一遍测试才能convert
    test_model_after_quant(config, quant_model, test_dataloader)
...
if __name__ == '__main__':
    ...
    test_model(config, model, test_dataloader)
    print('****************Start to quanti...******************')
    if config['ptq_type'] == 'ptdq':
        quant_model = ptdq(config, model)
    elif config['ptq_type'] == 'ptsq':
        quant_model = ptsq(config, model, test_dataloader)
    print('****************quanti over******************')
    print('****************test quanti...******************')
    if config['ptq_type'] == 'ptdq':
        print(f"PTQ type is {config['ptq_type']}")
    elif config['ptq_type'] == 'ptsq':
        print(f"PTQ type is {config['ptq_type']}")
    test_model_after_quant(config, quant_model, test_dataloader)
    print('***************test quant weight****************')
    #print(torch.int_repr(quant_model.weight()))
